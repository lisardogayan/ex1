...
export class Services {
    ...
    // all records
    public addAllColumnHeaders(arr: any, table: any) {
        var columnSet = [],
    ...
    public buildHtmlTable(arr: any) {
        ...
        for (var i = 0, maxi = arr.length; i < maxi; ++i) {
            ...
            for (var j = 0, maxj = columns.length; j < maxj; ++j) {
                ...
                var cellValue: string= arr[i][columns[j]].toString();
                //td.appendChild<Text>(document.createTextNode(arr[i][columns[j]] || ''));
                td.appendChild<Text>(document.createTextNode(cellValue || ''));
